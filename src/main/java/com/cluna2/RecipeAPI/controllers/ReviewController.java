...
public class ReviewController {
    ...
    public ResponseEntity<?> postNewReview(
        ...
        } catch (NoSuchRecipeException | UserReviewException | NoSuchUserException e) {
    ...
    public ResponseEntity<?> deleteReviewById(@PathVariable("id") Long id) {
        try {
            ...
            return ResponseEntity.ok("The review with id: " + deletedReview.getId() +
                    " by username: " + deletedReview.getUser().getUsername() +
                    " is deleted. ");
        } catch (NoSuchReviewException e) {
            return ResponseEntity.badRequest().body(e.getMessage());
        }
    ...
    @GetMapping("/{username}")
    public ResponseEntity<?> getReviewsByUsername(@PathVariable(name = "username") String username) {
        try {
